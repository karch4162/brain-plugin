...
5. ✅ **DONE (2026-07-17, v0.2.5)** — **`/brain:promote` helper — draft→trusted (spec'd 2026-06-24).** Built as `skills/promote/SKILL.md` (no `commands/` wrapper — those were removed in 0.2.3 to fix double registration). Follows the spec below, plus two additions from the 2026-07-17 sports-management memory-ecosystem audit: a **14-day promote-or-drop TTL** for stale drafts (staging is a queue, not a home — the audit found 15 drafts stranded since 7-05, two of which had to be independently re-learned into auto-memory), and a **de-dup rule** (downstream copies in project auto-memory/CLAUDE.md become pointers to the promoted wiki note; wiki is canonical). Original spec: Automates the mechanical toil the README "Promoting a draft → trusted" section now describes, **keeping the human as the gate** (never auto-promotes). Flow: (a) list `wiki/_drafts/` (optional arg = one draft or all); (b) **validate/complete frontmatter** — ensure `owner` (default to git user), a `source:` anchor, `last_verified`=today, `confidence`; flag anything missing for the human to confirm; (c) **pick the target** — `wiki/<area>/` from the note's id-prefix/tags, or `wiki/bridges/` for a cross-repo contract; ask if ambiguous; (d) move out of `_drafts/`, add a line to `wiki/index.md`, ensure a `Code:` `[[_COMMUNITY_*]]` link line (warn if absent); (e) **stage for a PR — do not commit** (trusted notes are intentionally off `.saveinclude`; the PR *is* the promotion, per the push guardrail). Complements `/brain:freshness` (rot review) — promote handles the draft lifecycle. Pairs with #8: #8 stops junk drafts at the source, promote graduates the good ones.
...
## How to continue
- **Harden the plugin:** #1/#2/#6/#7 shipped (v0.2.0). Open: #3 (init remote handling), #4 (backfill personal-brain CLAUDE.md), #8 (docs-ingest allow-list). #5 (`/brain:promote`) shipped in v0.2.5.
